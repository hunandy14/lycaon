...
/** 觀戰/陰間聊天室：GET 歷史 + SSE append；暱稱存 localStorage，首次送出前詢問。
 *  base='watch'（同樂邀請連結）走 /api/watch，base='ghost'（陰間連結）走 /api/ghost 並帶 scope 分房
 *  （陰間 stream 兩房都轉發，靠 scope 在前端篩自己這間）。
  ...
  return (
    <section className="panel chat-panel" style={{ marginTop: 10, marginBottom: 30 }}>
      <div className="panel-title" style={{ fontSize: '0.95rem' }}>
        {title ?? '💬 聊天室'} {live && !disabled ? <span className="live-dot on">●</span> : null}
      ...
      <div ref={listRef} className="chat-list" onScroll={handleScroll}>
        ...
        {messages.map((m) => (
          <div key={m.id} className="chat-row">
            <span className="chat-nick">{m.nick}{m.isGm && <span className="chip chip-idiot" style={{ marginLeft: 4 }}>GM</span>}</span>
