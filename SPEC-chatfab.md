...
- [x] 2. **WatchPage 接 1 球**：移除內嵌聊天，接 ChatFab(slot 0)+ChatRoom；未讀數邏輯上線。
- [x] 3. **GhostPage 接 2 球**：移除兩個內嵌聊天，陰間/陽間雙球；同頁單面板互斥。
- [x] 4. **GamePage GM 雙球**：移除 💬 按鈕與 GmChatSheet 引用，gm 模式雙球（含原 15s 輪詢
      顯示條件與未讀搬遷）；刪除 WatchChat.tsx、GmChatSheet.tsx 與孤兒 CSS。
- [x] 5. **收尾總驗**：CLAUDE.md client 段更新（聊天 FAB 描述）；全閘門 + `npm run build`；
